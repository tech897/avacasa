...
export class SearchParser {
  ...
  private extractPropertyType(query: string): string | undefined {
    ...
    const sortedTypes = Object.keys(this.propertyTypeMap)
  ...
  private extractLocation(query: string): string | undefined {
    ...
    const commonLocations = [
      'goa', 'bangalore', 'mumbai', 'delhi', 'chennai', 'hyderabad', 'pune', 'kolkata',
      'coorg', 'kodagu', 'mysore', 'ooty', 'lonavala', 'mahabaleshwar', 'shimla',
      'manali', 'mussoorie', 'nainital', 'darjeeling', 'gangtok', 'munnar', 'wayanad',
      'chikmagalur', 'sakleshpur', 'yercaud', 'kodaikanal', 'coonoor', 'karjat',
      'khandala', 'alibag', 'kashid', 'diveagar', 'gokarna', 'hampi', 'badami'
    ]
    ...
    for (const pattern of this.locationPatterns) {
      ...
      if (match && match[1]) {
        ...
        const excludeWords = ['properties', 'property', 'homes', 'villas', 'plots', 'apartments', 'with', 'having', 'good', 'best', 'new', 'old']
  ...
  private extractPriceRange(query: string): { minPrice?: number; maxPrice?: number } {
    const result: { minPrice?: number; maxPrice?: number } = {}

    // Under/below patterns
    const underPattern = /(?:under|below|less\s+than|maximum|max|up\s+to)\s+(?:rs\.?\s*)?(\d+(?:\.\d+)?)\s*(crore|crores|cr|lakh|lakhs|lac|lacs|thousand|k|million)/gi
    ...
    // Above/minimum patterns
    const abovePattern = /(?:above|over|more\s+than|minimum|min|starting\s+from|from)\s+(?:rs\.?\s*)?(\d+(?:\.\d+)?)\s*(crore|crores|cr|lakh|lakhs|lac|lacs|thousand|k|million)/gi
    ...
    // Between patterns
    const betweenPattern = /(?:between|from)\s+(?:rs\.?\s*)?(\d+(?:\.\d+)?)\s*(crore|crores|cr|lakh|lakhs|lac|lacs|thousand|k|million)\s+(?:to|-|and)\s+(?:rs\.?\s*)?(\d+(?:\.\d+)?)\s*(crore|crores|cr|lakh|lakhs|lac|lacs|thousand|k|million)/gi
  ...
  private extractAmenities(query: string): string[] {
    ...
    for (const pattern of this.amenityPatterns) {
      ...
      if (pattern.test(query)) {
        const amenity = pattern.source.replace(/\\s\+/g, ' ').replace(/\\\\/g, '')
  ...
  private extractKeywords(query: string, parsed: ParsedQuery): string[] {
    ...
    if (parsed.bedrooms) {
      remainingQuery = remainingQuery.replace(/\d+\s*(?:bhk|bedroom|bed|br)/gi, '')
    }
    ...
    if (parsed.location) {
      remainingQuery = remainingQuery.replace(new RegExp(parsed.location, 'gi'), '')
    }

    // Remove price-related terms
    remainingQuery = remainingQuery.replace(/(?:under|below|above|over|between|from|to|rs\.?|\d+(?:\.\d+)?\s*(?:crore|lakh|thousand|cr|k))/gi, '')

    // Remove common stop words
    const stopWords = ['in', 'at', 'near', 'with', 'having', 'and', 'or', 'the', 'a', 'an', 'is', 'are', 'for', 'of', 'to', 'from']
  ...
  public summarize(parsed: ParsedQuery): string {
    ...
    if (parsed.minPrice || parsed.maxPrice) {
      if (parsed.minPrice && parsed.maxPrice) {
        parts.push(`₹${this.formatPrice(parsed.minPrice)} - ₹${this.formatPrice(parsed.maxPrice)}`)
