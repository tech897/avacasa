...
export async function GET(request: NextRequest) {
  try {
    ...
    if (parsed.confidence < 30) {
      return NextResponse.json({
        ...
        data: {
          ...
          properties: [],
          fallback: true,
          message: "Query not clear enough for natural language processing. Try using specific terms like '2 BHK villa in Goa under 2 crore'."
        },
        pagination: {
          page: query.page,
          limit: query.limit,
    ...
    if (parsed.location) {
      ...
      // Filter properties whose related location name contains the parsed location text
      where.location = {
        name: { contains: parsed.location }
      }
    }
    ...
    // Note: Price filtering will be handled in application layer due to string storage

    // Add keyword search if any remaining keywords
    if (parsed.keywords && parsed.keywords.length > 0) {
      const keywordSearch = parsed.keywords.join(' ')
      where.OR = [
        { title: { contains: keywordSearch } },
        { description: { contains: keywordSearch } },
        { location: { name: { contains: keywordSearch } } }
      ]
    }

    // Execute the search query (get all matching properties first for price filtering)
    ...
    if (parsed.minPrice || parsed.maxPrice) {
      priceFilteredProperties = allProperties.filter(property => {
        ...
        // Debug logging
        console.log(`Property: ${property.title}, Price: ${property.price}, Parsed: ${priceNum}, Max: ${parsed.maxPrice}`)
        ...
        if (parsed.minPrice && priceNum < parsed.minPrice) {
          console.log(`${property.title} filtered out - price ${priceNum} < min ${parsed.minPrice}`)
          passesFilter = false
        }
        
        if (parsed.maxPrice && priceNum > parsed.maxPrice) {
          console.log(`${property.title} filtered out - price ${priceNum} > max ${parsed.maxPrice}`)
          passesFilter = false
        }
    ...
    console.log(`Total properties after filtering: ${priceFilteredProperties.length}`)
    ...
    try {
      await prisma.searchQuery.create({
        data: {
          ...
          results: 0, // Will be updated after we get results
