...
function createLocationFromProperty(property) {
  ...
  return {
    ...
    description:
      property["Info - Micromarket"] || `Properties in ${locationName}`,
    image:
...
async function importProperties() {
  ...
  const properties = [];
  const csvFilePath =
    "PropertyDetailsN/Properties Details_N - Main Project Details (1).csv";
...
async function processProperties(properties) {
  ...
  for (let i = 0; i < properties.length; i++) {
    ...
    try {
      console.log(
        `\n📝 Processing property ${i + 1}/${properties.length}: ${
          csvProperty["Project Name"]
        }`
      );
      ...
      // Parse property data
      const propertySlug =
        csvProperty["Listing Id"] || createSlug(csvProperty["Project Name"]);
      const coordinates = parseCoordinates(
        csvProperty["Latitude"],
        csvProperty["Longitude"]
      );
      ...
      const sizeRange =
        csvProperty["Size Range (in sq.ft)"] ||
        csvProperty["Sq.ft Onwards - Size"] ||
        "";
      ...
      const propertyData = {
        ...
        slug: propertySlug,
        description:
          csvProperty["About the Project"] ||
          csvProperty["Project Headline"] ||
          "Premium property development",
        price:
          parseNumericPrice(csvProperty["Numerical_Price"]) ||
          parsePrice(csvProperty["Starting Price (INR )"]),
        ...
        features: JSON.stringify(
          views.map((view) => ({ name: view, icon: "eye" }))
        ),
        ...
        unitConfiguration: csvProperty["Type - Unit Configuration"]
          ? JSON.stringify({
        ...
        legalApprovals: JSON.stringify([
          {
            ...
            status:
              csvProperty["RERA ID"] &&
              csvProperty["RERA ID"] !== "Not Available"
                ? "approved"
                : "pending",
          },
        ]),
        ...
        registrationCosts: JSON.stringify({
          ...
          registrationCost:
            csvProperty["Registeration Cost"] || "Contact for details",
          maintenanceCost:
            csvProperty["Maintainence Cost "] || "Contact for details",
        }),

        propertyManagement: csvProperty["Post Property management"]
          ? JSON.stringify({
        ...
        financialReturns: csvProperty["Financial Returns & Investment Benefits"]
          ? JSON.stringify({
              description:
                csvProperty["Financial Returns & Investment Benefits"],
        ...
        nearbyInfrastructure: JSON.stringify({
          ...
          attractions: [
            ...
            (attraction) => attraction.name && attraction.name.trim() !== ""
          ),
        }),

        plotSize: csvProperty["Project Land Size (Acres)"]
          ? `${csvProperty["Project Land Size (Acres)"]} acres`
          : null,
        constructionStatus:
          csvProperty["Current Status"] || "Under Construction",
        ...
        metaDescription:
          csvProperty["Project Headline"] ||
          `Discover ${csvProperty["Project Name"]} - a premium property development`,
        tags: JSON.stringify(
          [
      ...
      if (existingProperty) {
        console.log(
          `⚠️  Property with slug '${propertySlug}' already exists, updating...`
        );
      ...
      console.error(
        `❌ Error processing property '${csvProperty["Project Name"]}':`,
        error.message
      );
      errorCount++;
